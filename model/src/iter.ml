...
let init p =
  ...
  let clear (a : int array) =
    for i = 0 to len a - 1 do
      a.(i) <- 0
    done
  in
  let get (a : int array) i =
    if i < 0 then G.zero else if i >= len a then G.zero else a.(i)
  in
  let iteri (a : int array) (f : int -> int) =
    for i = 0 to len a - 1 do
      a.(i) <- f i
    done
  in
  let copy (t : int array) (f : int array) =
  ...
  let decode =
    ...
    let chien () =
      for n = 0 to p.n - 1 do
        if rhorner (G.antilog n) lambda = G.zero
        then (
